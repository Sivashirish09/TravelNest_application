...
# Use the maximum available options if this were a JVM bug.
# See https://github.com/gradle/gradle/issues/17845
...
# OS specific support (file extension mem problem, etc).
cygwin=false
msys=false
darwin=false
nonstop=false
...
if [ "$cygwin" = "false" -a "$darwin" = "false" -a "$nonstop" = "false" ] ; then
    ...
    if [ $? -eq 0 ] ; then
        if [ "$MAX_CAN_TRY_ULIMIT" != "false" ] ; then
...
# For Darwin, add options to compiler
if $darwin; then
...
if [ "$cygwin" = "true" -o "$msys" = "true" ] ; then
    ...
    # We build the extension for each file, see --script-classpath below
    # for more information.
    # set --
    # for f in "$APP_HOME"/lib/*.jar; do
    #   set -- "$@" "$f"
    # done
    # JVM_OPTS="$JVM_OPTS -cp \"$*\""
fi
